<?php
    ...
    if($weight == FALSE || $est_qty == FALSE){
        ...
        $query = mysql_query("INSERT INTO `seeds` (`id`, `date`, `tree_type_id`, `weight`, `est_qty`) VALUES (NULL, CURDATE(), '$tree_type_id', '$weight', '$est_qty')");

        //Assign the Seed Id
        echo $seed_id  = mysql_insert_id();
        
        // Insert the Data to `store` table
        if($query == TRUE){

            // Check if the tree_type is already in the store table
